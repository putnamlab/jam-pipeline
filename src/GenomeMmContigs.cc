...
void PrintOptions() {
  cerr << "Option values are:\n"<<
    ...
    "   -E {EdgesIn}     ["<< OptEdgesIn     <<"] Edge input file\n" <<
    "   -e {EdgesOut}    ["<< OptEdgesOut    <<"] Edge output file\n" <<
    ...
    "   -m {Min}         ["<< OptMin      <<"] Minimum total count in reads\n" <<
    "   -M {Max}         ["<< OptMax      <<"] Maximum  ''     ''  ''  ''  \n" << 
...
public:
	...
	unsigned visited:      1; // use in graph traversal to avoid adding another bit to Nodes structure 
...
void positionAddList(string posns) {
	...
	if (posns[0] == '*') {
		for (int i = 0; i < sizeof(usePosition)/sizeof(bool); i++) {
			usePosition[i] = true;
			if (debug.check('o')) cerr << "Added position " << dec << i << " to filter\n";
		}
		return;
	}
	size_t comma;
	for (comma = 0; comma != string::npos; posns = posns.substr(comma + 1)) {
		...
		if (debug.check('o')) cerr << "Added position " << dec << num << " from " << posns << " to filter\n";
		comma = posns.find(',');
	}
}
...
int SetupOptions(int argc, char**argv)
{
  ...
  OptEdgesIn     = "";        // -E <filename>
  OptEdgesOut    = "";        // -e <filename>
	OptWalkFile    = "";        // -w <filename>
	// OptPatterns    = "*";       // -p <pattern>[,<pattern>]* or just * (for all)
	OptPositions   = "3,12,21"; // -P <small_integer>[,<small_integer>]*
	OptAmbiguous   = false;     // -a
	OptMin         = 2;         // -m <num>
	OptMax         = ~0;        // -M <num>  // default max unsigned = no limit
    ...
    if (prefix == '-') {
      ...
      case 'H': {
				...
				if (prime < OptHashSize) {
					cerr << "Hash size " << dec << OptHashSize << " reduced to prime number "
							 << prime << "." << endl;
					OptHashSize = prime;
				}
      }
      ...
      case 'E':
				OptEdgesIn = argv[++i]; break;
      case 'e':
				OptEdgesOut = argv[++i]; break;
			case 'w':
				OptWalkFile = argv[++i]; 
				break;
				// case 'p':
...
void inline printFields(ostream &out,
												OligoHash &oh,
												Allelic side[],
												OligoHash::Oligo* op) {
...
inline OligoHash::Index insertOrDie(OligoHash &oh,
...
bool readEdgeRecord(istream &in, 
										Oligos::Oligo &kmer1,
										Oligos::Oligo &kmer2,
										unsigned &orient,
										unsigned &distance,
										unsigned &nreads)
{
	...
	while (in.getline(buf, BUFSIZE)) {
		// successfully got a line
		int parsed = sscanf(buf, "%lx %lx %u %u %u", &kmer1, &kmer2, &orient, &distance, &nreads);
		if (debug.check('e')) {
			cerr << "Got " << hex << kmer1 << " " << kmer2 << dec << " " << orient << " " << distance << " " << nreads << 
				" from line: " << buf;
		}
		if (parsed == 5) {
			return true;
...
// Always consider whether we're walking downstream on the original strand
// (original == true) or upstream on the opposite strand (original = false).
// May switch back and forth many times during a walk

inline OligoEdge *robustEdge(OligoEdges &edges) {
	if (!edges.size() || !(edges[0].nreads))
		// There are no good edges (or there were too many, discarded) in this direction
		return NULL;

	OligoEdge *greatest = &(edges[0]);
	unsigned runnerup = 0;
	unsigned lessersum = 0;
	unsigned i;
	// Compare first edge with zero or more additional edges
	for (i = 1; i < edges.size(); i++) {
		if (edges[i].nreads > greatest->nreads) {
			lessersum += greatest->nreads;
			runnerup = greatest->nreads;
			greatest = &(edges[i]);
		}
		else {
			lessersum += edges[i].nreads;
			if (edges[i].nreads > runnerup)
				runnerup = edges[i].nreads;
		}
	}
	if ((greatest->nreads > lessersum + 1) &&
			(greatest->dist != 0) &&
			(greatest->nreads >= (runnerup * 2))) {
		return greatest;
	}
	return NULL;
}

enum Strands { TOP = 0, BOTTOM = 1 };

inline OligoEdge *mutualEdge(OligoNode nodes[], unsigned source, Strands strand) {
	// BOTTOM is upstream relative to normalized source kmer; TOP strand is downstream
	if (debug.check('w'))
		cerr << "Finding mutualEdge: " << hex << nodes << " " << dec << source << " " << strand << endl;
	OligoEdges *edges = (strand == TOP)? &(nodes[source].down) : &(nodes[source].up);
	if (debug.check('w'))
		cerr << "Calling robustEdge on " << hex << edges << "...";
	OligoEdge *edge = robustEdge(*edges);
	if (debug.check('w'))
		cerr << " done.\n";
	if (!edge) 
		return NULL;
	OligoEdge *back = NULL;
	if (strand == TOP) {
		if (STRANDWARDS == edge->orient) {
			back = robustEdge(nodes[edge->sink].up);
		}
		else if (INWARDS == edge->orient) {
			back = robustEdge(nodes[edge->sink].down);
		}
	}
	else { // going bottom-strand upstream towards sink
		if (OUTWARDS == edge->orient) {
			back = robustEdge(nodes[edge->sink].up);
		}
		else if (BACKWARDS == edge->orient) {
			back = robustEdge(nodes[edge->sink].down);
		}
	}
	if (back && back->sink == source)
		return edge;
	else 
		return NULL;
}

inline void emit(ostream &out,
								 OligoHash &oh, Allelic side[], 
								 unsigned wi,
								 unsigned offset,
								 Strands strand) {
	char typechar;
	Oligos::Oligo w_norm = oh.getOligo(oh.hash[wi]);
	if (side[wi].visited) {
		cerr << "Error in trying to emit " << hex << w_norm << " again" << endl;
		return;
	}
	if (side[wi].partnered) {
		OligoSeq::Index pi;               // kmer partner's index
		...
		// typechar = typeCharByBV(side[wi].inLibs, side[pi].inLibs);
		out << dec << '1' << "\t" << offset << "\t" << strand << "\t";
		printFields(out, oh, side, oh.hash + wi);
		out << "\t" << dec << side[wi].pos 
				<< "\t" << side[wi].xormask
				<< "\t" << side[wi].flip
				<< "\t";
		printFields(out, oh, side, oh.hash + pi);
		out << endl;
	}
	else {
		// not partnered
		...
		// typechar = 'n';
		// }
		// if ((side[wi].inLibs >> NKIDS) == 3)
		// typechar = toupper(typechar);
		out << dec << '0' << "\t" << offset << "\t" << strand << "\t";
		printFields(out, oh, side, oh.hash + wi);
		out << endl;
	}
	side[wi].visited = 1;
...
void walk(ostream &out, 
					OligoHash &oh, Allelic side[], OligoNode nodes[], 
					OligoEdge *edge,
					unsigned offset,
					Strands strand) {
	// Traverse this edge (adding to the accumulated offset)
	// and print the node/kmer, 
	// then try and walk some more
	while (edge) {
		if (true) { // debug.check('w')) {
			cerr << "Walking edge to " << dec << edge->sink 
					 << "/" << hex << oh.getOligo(oh.hash[edge->sink]) << dec
					 << ", orient=" << edge->orient
					 << ", lib=" << edge->lib
					 << ", dist=" << edge->dist
					 << ", nreads=" << edge->nreads
		...
		if (side[edge->sink].visited) {
			cerr << "CYCLE: " << edge->sink << " already visited" << endl;
			return;
		}
		offset += edge->dist;
		if (((TOP == strand) && (STRANDWARDS == edge->orient)) ||
				((BOTTOM == strand) && (OUTWARDS == edge->orient))) {
			emit(out, oh, side, edge->sink, offset, TOP);
			edge = mutualEdge(nodes, edge->sink, TOP);
			strand = TOP;
		}
		else if (((TOP == strand) && (INWARDS == edge->orient)) ||
						 ((BOTTOM == strand) && (BACKWARDS == edge->orient))) {
			emit(out, oh, side, edge->sink, offset, BOTTOM);
			edge = mutualEdge(nodes, edge->sink, BOTTOM);
			strand = BOTTOM;
		}
		else { // die
			cerr << "Impossible strand " << strand << " & orientation " << edge->orient << " during walk\n";
			exit(-1);
		}
	}
}
...
int main(int argc, char *argv[]) {
  ...
  int filearg = 0;
	...
	if (OptEdgesIn.length()) {
		Oligos::Oligo oligo1, oligo2;
		unsigned orient, dist, nreads;
		igzstream edgesIn(OptEdgesIn.c_str());
		cerr << "Loading edges from " << OptEdgesIn << " ...";
		while (readEdgeRecord(edgesIn, oligo1, oligo2, orient, dist, nreads)) {
			if (debug.check('e'))
				cerr << "Loading edge " << hex << oligo1 << "\t" << oligo2
						 << dec << "\t" << orient << "\t" << dist << "\t" << nreads << endl;

			Oligos::Index id1, id2;
			if (oh.lookuploc(oligo1, id1) != OligoHash::FOUND) {
				cerr << "Failed to find entry for oligo1 in edge " << hex << oligo1 << "\t"
						 << oligo2 << dec << "\t" << orient << "\t" << dist << "\t" << nreads << endl;
				exit(-1);
			}
			if (oh.lookuploc(oligo2, id2) != OligoHash::FOUND) {
				cerr << "Failed to find entry for oligo2 in edge " << hex << oligo1 << "\t"
						 << oligo2 << dec << "\t" << orient << "\t" << dist << "\t" << nreads << endl;
				exit(-1);
			}
			nodes[id1].add_edge(id2, (OligoOrient) orient, 0, dist, nreads);
			edgeInserts++;
		}
		cerr << "Loaded " << dec << edgeInserts << " edges from " << OptEdgesIn << endl;
	}
	else {
			...
			if (!strcmp("/", argv[filearg])) {
				...
				while ((np = kmers.nextPos()) >= 0) {
					if (np > 0) {
						...
						if (prev != NULLINDEX) {
							if (debug.check('e')) {
								cerr << "adding edge between " << oh.getOligo(oh.hash[prev]);
								cerr << " and " << oh.getOligo(oh.hash[w_rep]) << endl;
							}
							edgeInserts++;
				...
				// now np < 0
				cerr << "done with " << argv[filearg] << ", #edgeInserts: " << dec << edgeInserts << endl;
				inputf.close();
			}
		}
	}
	...
	if (OptWalkFile.length()) {
		ofstream walkFile(OptWalkFile.c_str());
		Oligos::Index i;
		Oligos::Index ncontigs = 0;

		// Determine if node can be start of walk (only go in one direction)
		// Print header for contig ( > starting kmer & strand(+/-)
		// Walk (printing along the way)
		// When reach a dead end, loop that terminating read back to the start so it won't be walked again
		//     (it will have edges on both sides)

		// BUT these rules are asymmetric about starting vs. stopping walks
		// START when there is only one way to walk
		// but STOP when there's a dead end (symmetric to START)
		//          OR when there are inconsistent choices

		cerr << "Starting linear walks" << endl;
		for (i = 0; i < oh.Size; i++) {
			if ((! oh.hash[i]) || side[i].visited) 
				continue;
			bool walked = false;
			side[i].visited = 1; // to keep from walking to self
			Oligos::Oligo oligo = oh.getOligo(oh.hash[i]);
			// cerr << "Source: ";
			// printFields(cerr, oh, side, &(oh.hash[i]));
			// cerr << endl;
			OligoEdge *downEdge = mutualEdge(nodes, i, TOP);
			OligoEdge *upEdge = mutualEdge(nodes, i, BOTTOM);
			if (downEdge && !upEdge && !side[downEdge->sink].visited) {
				// walk downstream along the top strand
				cerr << "Walking down/right from " << dec << i << "/" << hex << oligo << "/" << oh.Bases(oligo) << endl;
				walkFile << dec << ">K." << ++ncontigs << endl;
				side[i].visited = 0; // clear visited so it can print
				emit(walkFile, oh, side, i, 1, TOP);
				walk(walkFile, oh, side, nodes, downEdge, /* offset */ 1, TOP);
				walked = true;
			}
			else if (upEdge && !downEdge && !side[upEdge->sink].visited) {
				// walk upstream (along the bottom strand, relative to starting kmer
				cerr << "Walking up/left from " << dec << i << "/" << hex << oligo << "/" << oh.Bases(oligo) << endl;
				walkFile << dec << ">K." << ++ncontigs << endl;
				side[i].visited = 0; // clear visited so it can print
				emit(walkFile, oh, side, i, 1, BOTTOM);
				walk(walkFile, oh, side, nodes, upEdge, /* offset */ 1, BOTTOM);
				walked = true;
			}
			// Setting of "visited" above was to keep from walking to self.
			// Un-visit now if we didn't walk after all
			if (!walked)
				side[i].visited = 0;
		}
		cerr << "Starting cycle-breaking walks" << endl;
		for (i = 0; i < oh.Size; i++) {
			if ((! oh.hash[i]) || side[i].visited) 
				continue;
			side[i].visited = 1; // to keep from walking to self
			...
			// cerr << endl;
			OligoEdge *downEdge = mutualEdge(nodes, i, TOP);
			if (downEdge && !side[downEdge->sink].visited) {
				// walk downstream along the top strand
				cerr << "Walking down/right from " << dec << i << "/" << hex << oligo << "/" << oh.Bases(oligo) << endl;
				walkFile << dec << ">C." << ++ncontigs << endl;
				side[i].visited = 0; // clear visited so it can print
				emit(walkFile, oh, side, i, 1, TOP);
				walk(walkFile, oh, side, nodes, downEdge, /* offset */ 1, TOP);
			}
				...
				OligoEdge *upEdge = mutualEdge(nodes, i, BOTTOM);
				if (!upEdge || side[upEdge->sink].visited)
					continue;
				// walk upstream (along the bottom strand, relative to starting kmer
				cerr << "Walking up/left from " << dec << i << "/" << hex << oligo << "/" << oh.Bases(oligo) << endl;
				walkFile << dec << ">C." << ++ncontigs << endl;
